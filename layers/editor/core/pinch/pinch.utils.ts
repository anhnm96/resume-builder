import type { PositionType, ReactZoomPanPinchContext } from '../../types'
import { isExcludedNode, roundNumber } from '../../utils'
import { checkZoomBounds } from '../zoom/zoom.utils'

export function isPinchStartAllowed(contextInstance: ReactZoomPanPinchContext, event: TouchEvent): boolean {
...
export function isPinchAllowed(contextInstance: ReactZoomPanPinchContext): boolean {
...
export function calculateTouchMidPoint(event: TouchEvent, scale: number, contentComponent: HTMLDivElement): PositionType {
...
export function calculatePinchZoom(contextInstance: ReactZoomPanPinchContext, currentDistance: number): number {
